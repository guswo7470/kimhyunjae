...
class FAT32:
    ...
    def scan(self, mode=''):
        for file in self.root:
            ...
            self.to_dic('data_set', None, file_entry)
            yield file_entry

    def get_freespace_set(self):
        for cluster_chain in self.get_chain():
    ...
    def get_md5(self, path=''):
        ...
        for file in self.scan():
            ...
            path_ = full_path.split(os.sep)
            if path in full_path and len(want_path) == len(path_):
                offset = self.start_root_directory + (file['entry_num'] - 2) * self.cluster_size
                file_buf = self.disk_handle.read_gen(offset, file['size_of_byte'])
                for buf in file_buf:
                    md5.update(buf)
                print(md5.hexdigest())
