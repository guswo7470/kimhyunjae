...
class KeywordSearch:
    def __init__(self, index_folder, file_num=1):
        ...
        self.cur1.execute("CREATE INDEX IF NOT EXISTS word_word_set_IDX ON word_set (word)")
    ...
    def make_index(self, path, binary=None):
...
class MakeIndex:
    ...
    def data_info(self):
        dic = {}
        try:
            if self.ext[1:] == 'ppt' or self.ext[1:] == 'xls' or self.ext[1:] == 'doc' or self.ext[1:] == 'pdf' or self.ext[1:] == 'hwp':
                ...
                for i in range(len(buf)):
                    ...
                    if len(dic[index_set]) > 100000:
                        ...
                        self.cur1.execute("INSERT INTO word_set (word, location) VALUES (?, ?)", (index_set, dic[index_set]))
                        self.conn1.commit()
...
if __name__ == '__main__':
    db_folder = 'D:\\X'
    handle = KeywordSearch(db_folder)
    start = timer()

    for i in handle.search('📌'):
        print(i)

    '''for path, buf in get_file("C:\\"):
        handle.make_index(path, buf)'''
